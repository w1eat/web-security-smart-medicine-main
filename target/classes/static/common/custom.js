...
function login() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data['code'] === 'SUCCESS') {
                setTimeout('reload()', 2000);
                sessionStorage.setItem('userName',loginAccount);
...
function uploadPhoto() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // 获取上传文件的文件名
            var uploadedFileName = file.name; // 这里获取文件名
            $("#img").attr('value', '/uploads/'+uploadedFileName); // 设置隐藏的 input 标签的值为文件名
        }
    });
}
...
// 获取元素
    var priceElement = document.querySelector('.cl-green');

// 获取价格文本
    var priceText = priceElement.textContent;
// 获取id参数的值
    let id = parseInt(url.searchParams.get('id'), 10);
    let medicineName =document.getElementById("medicineName").innerText;
    let medicinePrice = priceText.split('￥')[0].trim();
    let medicineNum = document.getElementById("buynum").value;
...
function deleteGoods() {
    let id =document.getElementById("medicineId").innerText;
    id=parseInt(id,10);
    $.ajax({
        type: "POST",
        url: "cart/delete",
